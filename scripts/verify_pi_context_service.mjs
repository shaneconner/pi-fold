...
const jitiPath = jitiPaths.find(existsSync);
if (!jitiPath) throw new Error("Could not resolve jiti for the active-context verifier");
...
const extensionRoot = join(projectRoot, ".pi", "extensions", "quorum");
...
const evidence = await jiti.import(join(extensionRoot, "evidence.js"));
const identity = await jiti.import(join(extensionRoot, "identity.js"));
const summarizerModule = await jiti.import(join(extensionRoot, "summarizer.js"));
// Mirrors the pi-fold package entry: summarizer translation, evidence ingestion, runtime registration.
const piFold = {
  registerPiFold(pi, options = {}, loadHostModule) {
    const {
    ...
    const configuredSummarizer = summarizeContextSpan ??
      summarizerModule.createSummarizeContextSpan(summarizer, loadHostModule);
    if (evidenceIngestion) {
      evidence.registerEvidenceIngestion(pi, {
        isMcpTool,
        entryTypePrefix: activeContextOptions.entryTypePrefix,
      });
    }
    return context.registerActiveContext(pi, {
      ...activeContextOptions,
...
async function gateLegacyBrandingReproduction() {
  ...
  assert(quorumReadOnlyTools.has("read") && quorumReadOnlyTools.has("wiki_read"));
...
async function gateSurfacingSelector() {
  const source = await readFile(join(extensionRoot, "lib", "surfacing.ts"), "utf8");
