...
pi-fold registers one tool, `pi_fold_context`, and gives the session's own agent eight actions over its transcript: `status`, `peek`, `fold`, `expand`, `refold`, `pin`, `unpin`, and `reboundary`, plus `unmark` under epoch scheduling. It also registers `/fold`, `/fold-editor`, `/fold-status` and `/fold-settings` for the human.
...
One call marks one span or many: `ids` with a `brief` marks a single span, and `marks: [{ids, brief}, ...]` carries up to 64 spans with their own briefs in one round trip. The brief is written at mark time and it is where every fold's brief comes from, because the commit is the only rewrite and a brief written after it would be a second one. A mark is editable until it commits: marking a span again replaces the standing mark, brief and all, at no cost, and `unmark` withdraws one entirely.
...
The agent's own actions come first. Through the context tool it folds any structurally closed span it judges stale, folds completed tool batches, consolidates two or more adjacent folds into a deeper one, expands any fold whose detail is needed again, dissolves a mis-cut boundary with `reboundary`, and pins entries that must stay raw. None of the ladder's pressure or width thresholds apply to these actions. Curation the agent cannot fix afterwards is curation it will not risk making, which is why `reboundary` is part of the ordinary surface rather than an extra.

Every fold, agent-made or automatic, obeys the same structural rules. Folds nest rather than partially overlap. Chapter folds align to closed user, assistant and tool-batch units, consolidations take two or more adjacent folds, and pinned or fresh evidence never folds. Size is where the two differ. The ladder keeps its own folds bite-sized, because it chose the boundary and nobody vouched for it, and a fold that hides more than it says is a fold nobody reads back. A span the agent names is taken whole at any size: it cut the boundary and wrote the brief, and a large fold stays cheap to read because peek takes an offset and a byte count, states the middle it omitted, and reads any child fold by id.

The automatic law folds only completed tool batches and structurally closed chapters, absorbs only the tool-result folds inside a chapter's own span, and treats fold placeholders as ordinary span material once the window carries `consolidateAfter` unpinned folds, which keeps the brief index tidy long before pressure matters. Briefs are bounded at 2,000 characters each, one cap for supplied and runtime-written briefs alike, so the visible index normally costs a low single-digit share of the window. Automatic foldability is membership, not position: a span folds because it is a completed tool batch or older material a chapter can compose, and what holds it back is the agent's pins, the blacklist, and the commit-time guard on the open turn.
...
Reading stays out of the way of both phases. `peek` is immediate, because it appends a tool result and never edits the prefix, so checking what a fold holds costs nothing a commit has to pay for. And a peek result is itself a completed read, so at the next commit it folds away automatically unless the agent committed to what it saw, which keeps look, decide, discard nearly free. There is no exemption for a span near the tail: `fold` always means mark, whatever the span's distance from the newest message, so what the agent gets back from the call never depends on arithmetic it cannot see.

The agent is told before the commit, while marking still changes the outcome. One warning band opens below the commit threshold, at the wider of a tenth of the serving budget and one recent request's growth, and stands until the budget is reached. The wider-of-two matters: a single large tool result can carry the window from under a fixed band to over the threshold in one arrival, and a band nothing lands inside never asks anything. Inside it, an ephemeral note at the tail of the window names the completed units the agent has not marked, largest first, states how many tokens the coming commit has to free, and shows the batch call that would take them. Whatever is still unmarked when the commit fires is chosen by age and folded with a brief the runtime writes.

Every mark answers with the same arithmetic, because an agent sees no occupancy and no threshold and cannot aim at a drop nobody states: what the next commit must free, what the standing marks will free when it runs, and what is left for the ladder. Both numbers are as of that moment and rise as new results arrive. When the answer says the marks cover the drop, the commit applies them and adds nothing of its own.
...
| `blacklistAutoFoldTools` | `new Set()` | The exception list. Every completed tool batch is foldable by the runtime on its own, without an agent mark; name a tool here and its results stay raw instead. Empty by default, because foldability is not the protection: pins and the commit guard are, and they apply to a blacklisted tool and an ordinary one alike. Pass a `ReadonlySet<string>`. Blacklisting is not free: the mass stays in the window until a chapter fold takes it, and a closed unit that alone exceeds the chapter cap is reclaimed whole rather than in pieces. |
