...
<picture>
  ...
  <img alt="The fold bar: forty cells of the serving budget, oldest on the left, with ticks at the aim and the commit point. Staged spans, raw material and fold placeholders fill it in three colours, and the label reads 42%, commit at 80%, 12 staged (9 spans, 3 truncations), 118k tokens, 3 folds (2 spans, 1 truncation) hide 412k tokens." src="https://raw.githubusercontent.com/shaneconner/fold/main/media/fold-bar-light.svg">
</picture>

The bar is the serving budget, forty cells wide, and it is a map: oldest material on the left, newest on the right, each cell one fortieth of the budget. The two ticks are the band: the left one is `minTarget`, where a commit aims to land, and the right one is `maxTarget`, where the commit fires. The fill is the provider's own count of what is in the window, never an estimate, so before the first response the row says `not measured yet` and draws nothing; inside the fill, where each thing sits is placed by the runtime's own pricing. Colours name what covers a stretch: raw material the runtime has not claimed; staged marks, the material the next commit collapses, in two shades because a span (a chapter of the conversation) and a truncation (a tool result) are two kinds of fold; low slivers where standing folds have already collapsed a stretch to a placeholder; and the theme's accent where you have pinned entries to keep them raw. A score, a quarter of a cell left empty on its right, is drawn where a staged mark or a standing fold ends, so the scores are the boundaries; a solid run is one mark or none, and a cell where several small marks end shows one score. The two ticks are drawn in the theme's text ink, which no fill class uses: a heavy bar at the commit point and a dotted one at the aim. The label repeats the percentage, names the commit point (or reads `at commit point` once the fill has crossed it and `commit held` once that count has been weighed), states how many marks are staged, of which kinds, and the tokens they hold, names how many entries are pinned when any are, and ends with what folding has already bought: the standing folds by kind, the nested count once consolidation has happened, and the tokens they hide from the window. Every token figure prices a pasted image as one image rather than as its base64 text, the same way the occupancy estimate does. Right after a commit the percentage is still the provider's last count, so the label says it is from before the commit until the next response measures the window again. If automatic folding ever stops, the row reads `FOLDING STOPPED` in place of the bar. On a truecolor terminal the fill classes take steps of Crameri's batlow colour map, with a rung between each pair so spans and truncations, and spans and consolidations, read apart; a consolidation sliver also stands a step taller. The kind words on the label wear the same inks as their cells, so the label is the legend. Elsewhere the classes use the theme's own colours.
