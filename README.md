...
The autonomous ladder is deliberately more conservative than the agent. It folds only completed read-only tool batches and structurally closed chapters, absorbs only the tool-result folds inside a chapter's own span, and creates folds of folds only past explicit thresholds (more than ten visible collapsed folds, or measured pressure at or above 85% of the provider window), taking adjacent folds oldest first. Cadence and fresh-tail rules prevent repeated or premature automatic folding.
