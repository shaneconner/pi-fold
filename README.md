...
The automatic law folds only completed read-only tool batches and structurally closed chapters, absorbs only the tool-result folds inside a chapter's own span, and treats fold placeholders as ordinary span material once the stale region carries `consolidateAfter` unpinned folds, which keeps the brief index tidy long before pressure matters. Briefs are bounded at 1,200 characters each, so the visible index normally costs about 1% of the window. The three zones are what prevent premature automatic folding: automation reaches only the stale tail, and the fresh tail and the middle are off limits to it.
...
The package entry calls `registerPiFold(pi)` with the defaults below, and that is the whole surface: six options, and every other name is refused with the replacement named in the message. Hosts that call the named `registerPiFold` export directly may pass an options object. Replacement sets replace the defaults; they are not additive.
...
| `summarizer` | `"session"` | Brief generator: the live session model, `"deterministic"` for no model calls, or `{ provider, model, effort? }` resolved through Pi's model registry. |
...
| `autoFoldableTools` | `new Set(["read", "grep", "find", "ls"])` | Replacement set of tool names whose completed batches the runtime may fold on its own, without an agent mark. Defaults to Pi's built-in read-only tools. Pass a `ReadonlySet<string>`. |
