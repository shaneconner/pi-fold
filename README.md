...
| `summarizer` | `"session"` | Brief generator: the live session model, or `{ provider, model, effort? }` resolved through Pi's model registry. Those are the only two forms. Every summarizer failure falls back to a deterministic brief on its own, so there is no value that selects one. |
