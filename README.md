...
Native compaction is a good pattern. It is the right default for most sessions: it costs nothing to adopt, it needs no tool surface, and models are trained under it and are genuinely good at choosing what to write down. pi-fold is an alternative for the sessions where the summary is the thing that hurts. It is not a replacement, and there is no claim it suits every workflow.

It earns its place when:

- **The transcript is the record.** Long agentic runs where what happened, in what order, beside what else, is the thing you will need later. A summary transcribes conclusions, not adjacency, and adjacency is what a session asks about when it goes back.
- **You have to be able to check.** Every fold resolves to the exact bytes it replaced, verified by SHA-256. A brief is a claim the runtime can settle, not one the reader has to take. Work under review, or work that has to be reconstructed months later, wants that property.
- **The window's price matters as much as its size.** Providers that charge a higher rate above a long-context boundary bill on which side of the line a request lands, not on how much was sent. A runtime that holds the window inside a band decides that, and holding the band is what folding does. On the twelve-run [ephemeral retrieval](https://doi.org/10.5281/zenodo.22143915) campaign, the folding arm never crossed the boundary and the compacting arm crossed on about half its requests; strip the surcharge and the two arms sit within about twelve percent of each other.

Native compaction remains the better choice when the session never fills its window, when the summary genuinely holds everything that will be asked for, or when a deployment cannot spend the tool surface. Folding is also not free: it costs a tool, a store on disk, and an agent that occasionally reads its own history.

### It pairs with a long-term memory store

Rotation and retention are different jobs. Rotation gets stale material out of the window. Retention keeps what the session learned. A handoff summary does both in one pass, which is why it is good and also where it runs out: it is durable context living in exactly one record, curated once under time pressure, and bounded by the window it has to fit inside.

Folding is the rotation half, and only that half. It is lossless, so nothing the session learned is destroyed, but a lossless record only pays when the reader goes and reads it, and reads the right part. That is where folding alone is exposed. A store answers exactly that: many records rather than one, each at its own address, written as the work happens, and it outlives the session that wrote it. An addressed store is also a better thing to read than a raw transcript.

The [rotation and retention](https://doi.org/10.5281/zenodo.22146898) campaign measured the pairing. Given the same external store, native compaction and folding answered the withheld exam identically, with no wrong answers on either. What separated them was the bill, and the folding arm paid about half. The store is what makes mechanical eviction safe, and mechanical eviction is the cheap way to hold a window.

[pi-canon](https://github.com/shaneconner/pi-canon) is the store this line built, developed alongside pi-fold and kept a separate package on purpose: the two jobs are separate, so the two packages are. Nothing here depends on it. The fold asks a store for two properties only, somewhere durable to put knowledge and an address the agent can follow back, and any long-term memory offering both should compose the same way. Only pi-canon was tested here.
...
Both pi-fold end-block turns included context recovery. The agent made five context calls in repetition 1 and nine in repetition 2 before answering. At the answering requests, the corrected carriage sweep classified 53 of 60 expected end-block values as visible raw and seven as recoverable but not visible, all seven in repetition 1. Those seven answers were correct, but the final projection does not identify how their bytes reached the answer: 60 of 60 is the scored outcome, not proof that recovery made the whole ledger visible. All eight withdrawn correction values were visible raw, and the agent selected the correction every time. The same 102-row certification found one ordinary miss with its expected value visible in a brief and the other recoverable but not visible, while the same matched control in both runs had its value absent from the answering branch.

There is no direct cross-arm end-block score and no cost ratio from this campaign: neither native run completed, and native repetition 1 is excluded from cost comparison because of the harness runaway. Two attempts do not estimate a population failure rate. The portable result contains all 102 rows and the source artifact hashes: [hidden-mass-results.json](https://github.com/shaneconner/pi-fold/blob/main/docs/fold_vs_compaction/hidden-mass-results.json). The full chronology and stop rule are in the [experiment log](https://github.com/shaneconner/pi-fold/blob/main/docs/fold_vs_compaction/experiment-log.md). The campaign is written up as its own paper, with the frozen plan and seeds, the sealed evidence set, and SHA-256 manifests deposited beside it: [doi.org/10.5281/zenodo.21980747](https://doi.org/10.5281/zenodo.21980747). The story of what it caught is on [Medium](https://medium.com/@shane.conner/the-summary-kept-the-story-it-threw-away-the-key-feafdf5f4132).
...
The runtime leads here: it marks as the session works, event by event, and the agent's briefs ride the marks. It commits the epoch on one condition: occupancy reaching `maxTarget` of the serving budget. Nothing else fires a commit automatically. Each automatic commit carries a depth bound: it applies marks stalest first and stops once the window is back at `minTarget`, so the newest events stay raw and a mark past the bound simply waits for the next epoch. The gap between the two lines is the hysteresis, which is what makes the spacing between events structural rather than hoped for.

Reading stays out of the way of both phases. `peek` is immediate, because it appends a tool result and never edits the prefix, so checking what a fold holds costs nothing a commit has to pay for. And a peek result is itself a completed read, so at the next commit it folds away automatically unless the agent committed to what it saw, which keeps look, decide, discard nearly free. There is no exemption for a span near the tail: a cut is a mark whatever the span's distance from the newest message, and it moves nothing until the commit.

The agent is told as the cuts stage, while a brief still changes what a fold will say. Once three or more staged folds are waiting without an agent brief, one bounded notice, at most 1,200 bytes, is appended once per frozen projection: it names the cut folds with their kinds and estimated sizes, and shows the `brief` call that annotates them. It is appended rather than re-rendered, so it never moves, never costs the cache, and never asks twice for a brief the agent has written. Whatever is still unbriefed when the commit fires folds with the brief the runtime wrote.

Every curation action answers with the same arithmetic, because an agent sees no occupancy and no threshold and cannot aim at a drop nobody states: what the next commit must free and what the standing marks will free when it runs. Both numbers are as of that moment and rise as new results arrive.

Protection is unchanged. The frontier never cuts protected evidence, and a pending mark that has become protected refuses at commit with a message naming it, leaving the rest of the epoch to apply. `pi_fold_context {"action":"status"}` reports the pending marks under `automatic.scheduling` with their origin, the estimated share of the serving budget a commit would free, and the estimated prefix tokens it would rewrite.
...
| --- | --- | --- |
| `thresholds` | `{ maxTarget: 0.80, minTarget: 0.40, consolidateAfter: 10, minFoldChars: 8000 }` | The band, set whole or not at all. The first two are proportions of the serving budget: fold at `maxTarget`, fold down to `minTarget`. Against a 272,000-token descriptor the defaults hold the session in roughly a 100k to 250k band: the trigger fires at 80% and the session runs one epoch's inflow past it before the commit lands, and the aim leaves about 100k of the freshest context raw across the whole cycle. `consolidateAfter` is a positive integer count of visible roots per parent owed. `minFoldChars` is the size in characters a fold has to reach to be worth making, and the same number is the floor a gap between two folds has to clear to stay raw: anything smaller is absorbed by the fold beside it. Characters rather than tokens because a token means a different amount of text on every wire. Validated atomically at registration, and an impossible policy is refused by name rather than clamped. User policy only: no agent action can read it back as a mutable surface, though status reports the values. |
...
| `toolFoldThreshold` | off | The tool-call diet. A share strictly between 0 and 1: tool results sitting in the oldest that-share of the projected window are clipped in view at each commit, the identifying head kept and the full bytes peek-recoverable behind the entry id the marker names. Off when absent, because a session whose tool results are small pays the clip for nothing. |
| `postFoldNotice` | `true` | The invitation switch. By default every fold announces itself with a standing invitation for the agent to improve the brief. Pass `false` to silence it: the runtime still cuts and still commits, and every fold goes out with the deterministic brief the runtime writes from the span itself. This is the shape that won the campaign reported above, and it is the tradeoff stated plainly, since an agent never told a fold happened is one that will not think to annotate it. The agent verbs stay on the tool either way. |
...
- Remove `guidance`. The option is refused by name: it switched the copy that taught the agent to choose spans, and the agent no longer chooses spans. The post-fold notice that replaced it is on by default, because a fold the agent is never told about is one it will not think to brief. It became optional in 3.1: `postFoldNotice: false` runs the deterministic shape the campaign measured.
...
- The default `thresholds` are `maxTarget: 0.80` and `minTarget: 0.40`, which against a 272,000-token descriptor holds a session in roughly a 100k to 250k band.
...
- The default `thresholds.minTarget` is now `0.40`. A host supplying the whole threshold object keeps its declared value.
